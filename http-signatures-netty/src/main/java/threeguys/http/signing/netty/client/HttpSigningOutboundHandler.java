...
public class HttpSigningOutboundHandler extends ChannelOutboundHandlerAdapter {
    ...
    public HttpSigningOutboundHandler(HttpSigner signer) {
        this.signer = signer;
    }
    ...
    public void write(ChannelHandlerContext ctx, Object msg, ChannelPromise promise) throws Exception {
        if (msg instanceof HttpRequest) {
            ...
            String sig = signer.sign(req.method().name(), req.uri(), new NettyHeaderProvider(req.headers()));
            req.headers().add(Signatures.HEADER, sig);
        }
